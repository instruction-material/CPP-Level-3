	...
	std::vector<Token> scan() {
		...
		while (!isAtEnd()) {
			...
			if (std::isdigit(static_cast<unsigned char>(current))) {
				tokens.push_back(scanNumber(current));
				continue;
			}
			if (current == '"') {
...
private:
	...
	Token scanStringLiteral() {
		...
		if (isAtEnd()) {
			return { TokenType::Unknown, lexeme, startLine };
		}

		advance();
		return { TokenType::StringLiteral, lexeme, startLine };
	}
