...
class InventoryIndex {
  public:
    ...
    project_names(const std::vector<Item>& rows) const {
	 ...
	 */
    std::vector<std::string>
    join_suppliers(const std::map<int, std::string>& supplier_by_id) const {
        ...
        for (const Item& item : items_) {
            ...
            row << item.id << " | " << item.name << " | " << item.quantity
                << " | " << supplier->second;
...
void load_sample_inventory(InventoryIndex& inventory) {
    inventory.add(
        {DEBUG_CABLE_ID, "debug cable", TOOLS_CATEGORY, DEBUG_CABLE_QUANTITY});
    inventory.add({SENSOR_KIT_ID, "sensor kit", ELECTRONICS_CATEGORY,
                   SENSOR_KIT_QUANTITY});
    inventory.add(
        {HEX_DRIVER_ID, "hex driver", TOOLS_CATEGORY, HEX_DRIVER_QUANTITY});
    inventory.add({BREADBOARD_ID, "breadboard", ELECTRONICS_CATEGORY,
                   BREADBOARD_QUANTITY});
}
...
int main() {
    ...
    std::cout << "Duplicate accepted? "
              << (inventory.add({DEBUG_CABLE_ID, "duplicate", TOOLS_CATEGORY,
                                 DUPLICATE_QUANTITY})
                      ? "yes"
                      : "no")
              << "\n";
